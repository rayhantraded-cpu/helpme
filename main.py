...
CATEGORY_TIPS = {
    ...
    "🚀 مهام تطويرية": [
        ...
        "تخصيص وقت لممارسة هواية يدوية لتفريغ طاقتك الإبداعية."
    ],
    "📚 مهام تعليمية": [
...
def main(page: ft.Page):
    ...
    def refresh_category_tasks_view():
        ...
        if not filtered:
            ...
            for t in filtered:
                # إصلاح طريقة فتح الروابط لتتوافق مع أندرويد
                task_url = t.get("link", "")
                ...
                if task_url:
                    trailing_button = ft.IconButton(
                        icon=ft.icons.LINK,
                        tooltip="فتح الرابط",
                        on_click=lambda e, u=task_url: page.launch_url(u)
                    )
    ...
    bottom_navigation_bar = ft.Row(
        [
            ...
            # تم تعديل زر الخروج ليظهر رسالة بدلاً من إغلاق النافذة لتوافق أندرويد
            ft.ElevatedButton("خروج", on_click=lambda e: page.show_snack_bar(ft.SnackBar(content=ft.Text("للخروج، استخدم زر الرجوع في هاتفك."), open=True)), style=ft.ButtonStyle(bgcolor=ft.colors.RED_600, color=ft.colors.WHITE), expand=True),
